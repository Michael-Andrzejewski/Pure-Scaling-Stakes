# Pure Scaling Stakes

By **Soareverix**.

A Balatro mod that adds 5 new stakes that only change blind scaling and never add debuffs. All stakes play like White Stake — no eternal, rental, or perishable jokers — just different score requirement curves.

## Stakes

| Stake | Scaling | Notes |
|-------|---------|-------|
| Soft-White | Level 1 (eased) | Antes 1–3 all use Ante 1 chips; scaling shifted back 2 antes |
| White-Blue | Level 2 | Red Stake's blind scaling, no debuffs |
| White-Red | Level 3 | Green Stake's blind scaling, no debuffs |
| White-Black | Level 4 | Cryptid Pink Stake's blind scaling, no debuffs |
| White-Void | Level 5 | Cryptid Verdant Stake's blind scaling, no debuffs |

## Requirements

- [Balatro](https://store.steampowered.com/app/2379780/Balatro/)
- [Lovely](https://github.com/ethangreen-dev/lovely-injector) (>= 0.8) — the mod injector that makes Balatro modding possible
- [Steamodded](https://github.com/Steamodded/smods) (>= 1.0.0-BETA) — the modding framework/API

Cryptid is **not** required, but this mod is fully compatible with it.

## Installation

1. Install Lovely and Steamodded if you haven't already.
2. Copy the `ScalingStakes` folder into your Balatro mods directory:
   ```
   %AppData%/Balatro/Mods/
   ```
3. Launch Balatro. The 4 new stakes will appear in the stake selection screen.
